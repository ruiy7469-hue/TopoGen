...
from src.sumo_logic import *
from src.ai_logic import *
from src.sumo_logic import TopoGenSkillLibrary
...
def main():
    ...
    if st.session_state.step == 1:
        ...
        # Initialize the Skill Library (Acting Layer)
        skills = TopoGenSkillLibrary()
        if col_start.button(t("start_sim"), type="primary"):
            # 1. Prepare Data from Session State
            coords = st.session_state.net_data["coords"]
            matrix = st.session_state.net_data["matrix"]

            # 2. Start the Orchestration Pipeline using a status container
            with st.status("TopoGen Agent: Orchestrating Acting Layer Skills...", expanded=True) as status:
                ...
                skills.skill_network_synthesis(
                    coords,
                    matrix,
                ...
                st.write("Invoking Skill: Traffic Demand Mapping...")
                rou, add_det = skills.skill_demand_mapping(
                    "ai_network.net.xml",
                    st.session_state.user_flows,
                ...
                if st.session_state.tls_data:
                    ...
                    add_tls = skills.skill_temporal_logic_serialization(
                        st.session_state.tls_data,
                        filename="tls.add.xml"
                    )

                    # Deterministic Merging Logic: Integrating TLS into the .net.xml
                    if add_tls and sumo_bin:
                        st.write("Merging temporal logic into physical network artifacts...")
                        ...
                        try:
                            ...
                            add_tls = None  # Successfully merged, no need for additional file
                        except subprocess.CalledProcessError as e:
                            st.error(f"Compilation Error during TLS Merge: {e.stderr}")

                # Combine all supplementary artifacts (Detectors + TLS if not merged)
                add_files = []
                ...
                # --- Skill 4: Simulation Execution Orchestration ---
                if rou:
                    st.write("Invoking Skill: Simulation Orchestration...")
                    skills.skill_simulation_orchestration(
                        final_net_file,
                        rou,
                        final_add_file,
                        delay=sim_delay
                ...
                # Finalize the status UI
                status.update(label="Acting Layer Execution Complete!", state="complete")
